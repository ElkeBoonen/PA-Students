...
namespace AStar___DSPS
{
    class Maze
    {
        ...
        private int NextNode(int[] priorities, bool[] visited)
        {
            int min = Int32.MaxValue;
            int index = -1;
            for (int i = 0; i < priorities.Length; i++)
            {
                if (priorities[i] < min && !visited[i])
                {
                    min = priorities[i];
                    index = i;
                }
        ...
        {
            int value = (int)Math.Sqrt(_maze.Length);
            int x1 = current % value;
            int x2 = end % value;
            int y1 = current / value;
            int y2 = end / value;

            int heuristic = Math.Abs(x1 - x2) + Math.Abs(y1 - y2);
            return heuristic;
        ...
        public string AStar(int start, int end)
        {
            ...
            {
                //get next node 
                int node = NextNode(priorities, visited);
